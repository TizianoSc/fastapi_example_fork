...
class TestDatabase:
    def test_add(self, session, db):
        juan = db.add(session, juan_upsert)

        assert juan.padron == 1
        assert db.list(session) == [juan]

    def test_add_autoincrementa_padron(self, session, db):
    ...
    def test_list_vacio(self, session, db):
    ...
    def test_delete_no_existente(self, session, db):
        db.add(session, juan_upsert)

        with pytest.raises(HTTPException, match="404: Alumno not found"):
            db.delete(session, 2)

    def test_update_valido(self, session, db):
        db.add(session, juan_upsert)
        db.add(session, manuelin_upsert)

        manuelin_update = db.update(session, 2, AlumnoUpsert(nombre="Manuelin", apellido="Equis", edad=22))
        assert manuelin_update == Alumno(padron=2, nombre="Manuelin", apellido="Equis", edad=22)

    def test_update_no_existente(self, session, db):
        db.add(session, juan_upsert)
